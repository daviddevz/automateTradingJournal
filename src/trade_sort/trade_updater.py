...
class TradeUpdater:
    def __init__(self, journal_dict: dict, transaction_dict: dict) -> None:
        ...
        self.multi_leg: bool = False
        self.new_trade: bool = False
        ...
        self.exit_date: str = None
        self.trade_des: str = None
        self.open_price: str = None
        self.close_price: str = None
        self.comm_fees: str = None
        self.pL: str = None     
    ...
    def add_trade_to_journal(self) -> None:
        ...
        if self.multi_leg == False:
            ...
            self.trade_progress.add_single_leg_trade(self.transaction_trade_idx)
            
        # For multi leg trades
        elif self.multi_leg == True:
            ...
            self.trade_progress.add_multi_leg_trade(self.journal_trade_idx)
    ...
    def update_trade_in_journal(self) -> None:
        ...
        self.trade_progress.close_trade(self.journal_trade_idx)
    ...
    def update_comm_fees(self) -> str:
        if self.new_trade == True and self.multi_leg == False:
    ...
    def reinitialize_attr(self) -> None:
        ...
        self.trade_progress.update_attr(self.journal_dict_, self.transaction_dict_)
        self.trade_validator.update_attr(self.journal_dict_, self.transaction_dict_)

    def update_trades(self) -> dict:   
        for idx in range(len(self.transaction_dict_['Description']), -1, -1):
            self.reinitialize_attr()
            self.trade_progress.update_progress(self.journal_trade_idx)
            trade_description: str = self.transaction_dict_['Description'][idx]
            self.multi_leg = self.trade_validator.is_trade_multi_leg(trade_description, idx)
            self.transaction_trade_idx = idx

            if self.trade_validator.trade_exists(self.multi_leg) == True:
